...
"""
"""
思路：滑动窗口，r往右走，同时统计另一个字符的数量，当另一个字符的数量超过k时，l往右走，遇到另一个字符就--，然后就能确定当前能
满足的最大距离，然后再和已有的答案取max
"""
    ...
    def maxConsecutiveAnswers(answerKey, k):
        ...
        def get_count(char):
            ...
            for r in range(len(answerKey)):
                total += answerKey[r] != char
                while total > k:
                    total -= answerKey[l] != char
                    l += 1
                ans = max(r - l + 1, ans)
            return ans

        return max(get_count('T'), get_count('F'))
