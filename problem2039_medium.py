...
同时给你一个二维整数数组edges，其中edges[i]=[ui,vi]表示服务器ui和vi之间有一条信息线路，在一秒内它们之间可以传输任意数目的信息。
...
"""
"""
思路：先bfs求到每一个点的最短距离，然后分类讨论，要是在patience之前就收到则2*dist+1就空闲了；否则会重发，
计算重发的最大次数*重发的时间间隔+2*dist+1就是结果
"""
