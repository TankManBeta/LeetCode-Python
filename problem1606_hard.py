...
给你一个 严格递增 的正整数数组 arrival ，表示第 i 个任务的到达时间，和另一个数组 load ，其中 load[i] 表示第 i 个请求的工作量
...
"""
思路：
（1）空闲服务器通过available维护，busy维护服务器完成时间以及编号，requests维护请求次数，每次来一个请求时，比较busy的第一个能否
在新任务来之前完成旧任务，如果可以就从busy中去掉，并放入available，继续判断。如果available为空，就丢弃当前请求；否则查找
available中不小于i%k的最小元素，如果找不到就把最小的服务器给新任务。设处理请求的服务器的编号为 jj，那么将 requests[j] 加 1，
并且将该服务器从available移除，然后将服务器j放入 busy 中，对应的处理结束时间为arrival[i]+load[i]。
（2）设在第 i 个请求到来时，编号为 id 的服务器已经处理完请求，那么将 id 从 busy 中移除，并放入一个不小于 i 且同余于 id 的编号，
