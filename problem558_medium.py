...
"""
"""
思路：node1为叶子节点时：如果 node1的值为 1，那么 node1∣node2=node1。否则 node1∣node2=node2。node2为叶子节点时：如果node2的值
为 1，那么 node1∣node2=node2。否则 node1∣node2=node1。两者都不是叶子节点时：那么分别对两者的四个子节点来进行对应的分治处理——
分别进行合并操作，然后再判断合并后的四个子节点的对应区域是否都为一个全 0 或者全 1 区域，如果是则原节点为叶子节点，否则原节点不是
叶子节点，且四个子节点为上面合并操作后的四个对应子节点
"""
