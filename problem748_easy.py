...
"""
"""
思路：统计词频，然后再words中count字典的key，数字小则说明不行，而且要最短，就先用lambda对words排个序即可
"""
...
class Solution(object):
    ...
    def shortest_completing_word(license_plate, words):
        ...
        :rtype: str
        """
        new_license = license_plate.lower()
        dict_license = {}
        for i in new_license:
            if 'a' <= i <= 'z':
                dict_license[i] = dict_license.get(i, 0) + 1

        words = sorted(words, key=lambda x: len(x))

        for word in words:
            flag = True
            for key in dict_license.keys():
                if word.count(key) < dict_license[key]:
                    flag = False
                    break
            if flag:
                return word
