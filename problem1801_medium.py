...
当前采购订单 buy 的价格，则匹配并执行这两笔订单，并将销售订单 sell 从积压订单中删除。否则，采购订单 buy 将会添加到积压订单中。
...
第 3 笔采购订单与价格最低的 1 笔销售订单匹配，销售订单价格为 25 ，从积压订单中删除这 1 笔销售订单。积压订单中不存在更多销售订单，
所以第 4 笔采购订单需要添加到积压订单中。最终，积压订单中有 5 笔价格为 10 的采购订单，和 1 笔价格为 30 的采购订单。
所以积压订单中的订单总数为 6 。
...
"""
"""
思路：两个队，一个大根堆一个小根堆，buy用大根堆，sell用小根堆，遍历每一个order，如果order是buy类型，则遍历sell，找到sell价格
低于order价格的订单，将这些全部处理了，如果处理完了所有的sell后order的订单还没处理完，那么将这些订单在放入buy中；如果order是
sell类型，处理方式相同。
"""
