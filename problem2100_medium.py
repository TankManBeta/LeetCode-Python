...
更正式的，第 i 天是一个合适打劫银行的日子当且仅当：security[i - time] >= security[i - time + 1] >= ... >= security[i] 
...
"""
"""
思路：从左往右维护左边递减的数组，从右往左维护右边递增的数组即可
"""
    ...
    def good_days_to_rob_bank(security, time):
        ...
        for i in range(1, n):
            if security[i - 1] >= security[i]:
                left_down[i] = left_down[i - 1] + 1
            if security[n - i] >= security[n - i - 1]:
                right_up[n - i - 1] = right_up[n - i] + 1
        for i in range(n):
